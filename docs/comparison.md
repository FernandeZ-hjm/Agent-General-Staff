...
| **Codex** | Agent 工程入口 | AGS 定义 Codex 在 preflight/solution/routing/review 中的角色和协议边界 |
...
三方协作模型中，Codex 承担 preflight、方案形成、任务分级路由和最终 review 的职责。
...
1. **执行前 gate** — 任务卡 validator + policy resolver 在 Agent 开始工作前检查
   "能不能做、怎么做、权限够不够"
2. **三段门槛** — 方案 OK ≠ 可以执行。需要显式任务卡指令才能进入路由
3. **执行后 receipt** — 每次执行都有结构化审计记录，不是聊天日志
4. **跨 runtime 协议** — 同一套任务卡、策略和验证规则对 Codex/Claude Code/Cursor
   通用
5. **技能治理** — 管理第三方技能的安装、审计和回滚，不自动安装不静默覆盖
...
AGS 2.0 Public Edition 是 **source-available** 软件，使用 `Agent Governance Suite
Public License 2.0`。它不是 OSI 认证的 open source 许可证。与上述工具（多数使用
MIT、Apache 2.0 等 OSI 许可证）的集成不改变各自的许可条款——AGS 治理的是工程流程，
不是代码的 license 派生关系。
