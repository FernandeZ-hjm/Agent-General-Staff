...
usage() {
    cat << 'EOF'
Usage: run-task-card.sh <task-card> [--check-only|--dry-run|--approve-writes] [--format text|json]

Compatibility wrapper for:
  ags run <task-card>

Options:
  --check-only       Stop after gate check; exit 0 if allowed/confirm, 1 if stopped.
  --dry-run          Output the full launch plan without executing.
  --approve-writes   Pass write approval to the resolver for Heavy tasks.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --check-only|--dry-run|--approve-writes)
            RUN_ARGS+=("$1")
            ;;
        --format)
            if [[ $# -lt 2 ]]; then
