...
for hook in "${obsolete_hooks[@]}"; do
    hook_path="$SUITE_ROOT/global-hooks/claude/$hook"
    if [[ -e "$hook_path" ]]; then
        red "  [FAIL] obsolete review hook still present: global-hooks/claude/$hook"
        obsolete_found=$((obsolete_found + 1))
    fi
done
if [[ $obsolete_found -eq 0 ]]; then
    green "  [OK] obsolete review hook scripts absent"
else
    ERRORS=$((ERRORS + obsolete_found))
fi
...
required_skills=(
    ...
    prompt-maker claude-delivery-report
...
PROMPT_TEMPLATE_DIR="$SUITE_ROOT/global-skills/prompt-maker/references/templates"
for level in light medium heavy; do
    template="$PROMPT_TEMPLATE_DIR/$level-task-template.md"
    if [[ -f "$template" ]]; then
        missing_fields=0
        for field in "${required_fields[@]}"; do
            if ! grep -Fq "$field" "$template" 2>/dev/null; then
                red "  [FAIL] prompt-maker template/$level: missing field '$field'"
                missing_fields=$((missing_fields + 1))
            fi
        done
        if [[ $missing_fields -eq 0 ]]; then
            green "  [OK] prompt-maker template/$level: all required fields present"
        else
            ERRORS=$((ERRORS + 1))
        fi
    else
        red "  [FAIL] prompt-maker template/$level: file not found"
        ERRORS=$((ERRORS + 1))
    fi
done
...
check "agent-project-profile.yaml.template" test -f "$SUITE_ROOT/project-integration/config/agent-project-profile.yaml.template"
...
check "governance/project-sync-registry.yaml" test -f "$SUITE_ROOT/governance/project-sync-registry.yaml"
...
echo "--- Script Syntax ---"
for script in bootstrap.sh install-suite-to-project.sh verify.sh diff-local.sh rollback.sh govern-new-skills.sh sync-project-workflow.sh run-task-card.sh context-memory.sh memory-start-context.sh suite-doctor.sh security-doctor.sh; do
...
if node --check "$SUITE_ROOT/scripts/configure-review-hooks.mjs" >/dev/null 2>&1; then
    green "  [OK] node --check scripts/configure-review-hooks.mjs"
else
    red "  [FAIL] node --check scripts/configure-review-hooks.mjs"
    ((ERRORS++)) || true
fi
if python3 -m py_compile "$SUITE_ROOT/scripts/sync-skill-aliases.py" >/dev/null 2>&1; then
    green "  [OK] python3 -m py_compile scripts/sync-skill-aliases.py"
else
    red "  [FAIL] python3 -m py_compile scripts/sync-skill-aliases.py"
    ((ERRORS++)) || true
fi

# 8.5 Registered project workflow sync gate
echo "--- Project Workflow Sync Gate ---"
if bash "$SUITE_ROOT/scripts/sync-project-workflow.sh" --check >/tmp/agent-project-workflow-sync-$$.log 2>&1; then
    green "  [OK] registered project workflow docs checked"
    cat /tmp/agent-project-workflow-sync-$$.log
else
    red "  [FAIL] registered project workflow docs are stale"
    cat /tmp/agent-project-workflow-sync-$$.log
    ((ERRORS++)) || true
fi
rm -f /tmp/agent-project-workflow-sync-$$.log
