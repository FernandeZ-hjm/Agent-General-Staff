...
# Check hooks
echo "--- Hooks ---"
for hook in "$SUITE_ROOT/global-hooks/claude/"*.mjs; do
    [[ -f "$hook" ]] || continue
    hook_name="$(basename "$hook")"
    diff_one_file "$hook" "$TARGET_HOME/.claude/hooks/$hook_name" "hooks/$hook_name"
done
for hook_name in leveled-review-gate.mjs review-baseline-snapshot.mjs codex-stop-review-adapter.mjs; do
    if [[ -f "$TARGET_HOME/.claude/hooks/$hook_name" ]]; then
        echo "  [LOCAL_ONLY_OBSOLETE] hooks/$hook_name"
        ((local_only++)) || true
    fi
done
...
for skill_name in "${obsolete_skills[@]}"; do
    if [[ -d "$TARGET_HOME/.agents/skills/$skill_name" ]]; then
        echo "  [LOCAL_ONLY_OBSOLETE] skills/$skill_name"
        ((local_only++)) || true
    fi
done
...
if [[ -d "$TARGET_HOME/.agents/skills" ]]; then
    for local_skill_dir in "$TARGET_HOME/.agents/skills/"*; do
        ...
        obsolete=0
        for obsolete_skill in "${obsolete_skills[@]}"; do
            [[ "$local_skill_name" == "$obsolete_skill" ]] && obsolete=1
        done
        [[ $obsolete -eq 1 ]] && continue
