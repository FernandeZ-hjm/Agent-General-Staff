...
对各宿主 GUI 的自动化操作。
这不表示 Cursor 原生探针已经通过；本次豁免只移除发布阻塞，失败证据仍保留。
...
- Cursor、CodeBuddy-Code、WorkBuddy 尚不具备与 Codex/Claude/OMP 等价的原生记忆
  close 证据。
...
`task`、`policy`、`project`、`session`、`verify` 与 `run` 是供 AGS
宿主/MCP 使用的隐藏 Machine CLI 合同，不属于人类命令面，也不要求用户直接调用。
