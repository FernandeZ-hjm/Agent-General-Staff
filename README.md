...
AGS 不允许 Agent 从用户的一句话直接跳到执行。2.7 采用**总路由模式**：每个用户请求先经过 `gate prompt-request` 分类意图、能力路由和价值路由，再决定是否进入任务卡管线。

```text
用户请求 → 总路由（意图分类 + 能力路由 + 价值路由）
  → 需要任务卡？
      是 → preflight → 方案形成 → 编译任务卡 → 校验 → 策略解析 → 执行 → 验证 → 回执 → 记忆
      否 → 放行普通响应
```
