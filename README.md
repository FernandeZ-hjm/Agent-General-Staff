...
仓库内置 `deny.toml`（RustSec 漏洞通报 + MIT/Apache-2.0/Unicode-3.0 许可证白名单 + crates.io 唯一来源），`cargo deny` 接入 CI 和 `scripts/verify.sh`，失败即关闭。依赖进项目之前先过安检。
...
AGS 2.5 在 CI 中通过 `ubuntu-latest`、`macos-latest` 和 `windows-latest` 三平台验证。
