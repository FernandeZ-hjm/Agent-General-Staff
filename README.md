...
v0.3.6 的 runtime workspace 只暴露十二个权威 Cargo package：
...
`workflow-sync-check` 的实现已收口到对应权威 module。0.3.6 只保留当前实际调用的
...
v0.3.6 只承诺下列当前命令面。已删除的旧命令、alias 和 plan-only 假动作不再作为兼容合同。
