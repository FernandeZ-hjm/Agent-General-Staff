...
当前源码发布候选是 **v0.4.1**；latest published release 是 **v0.4.0**。
...
v0.4.1 的 runtime workspace 只暴露十二个权威 Cargo package：
...
`workflow-sync-check` 的实现已收口到对应权威 module。0.4.1 只保留当前实际调用的
...
`ags setup --lifecycle-hosts` 只记录明确批准的宿主；`ags init` 为用户明确指定的
当前工作区消费该集合。公开版不会枚举或批量写入私有 managed-project fleet。
...
v0.4.1 只承诺下列当前命令面。已删除的旧命令、alias 和 plan-only 假动作不再作为兼容合同。
