...
function signedIndex(version = "0.4.13") {
...
test("switching versions writes previous pointer and never overwrites old content", async () => {
  ...
  try {
    ...
    const second = releaseMetadata({ version: "0.4.13" });
    ...
    const fetchImpl = async (url, options) => {
      const metadata = url.includes("v0.4.13") ? second : first;
    ...
    assert.equal(current.version, "0.4.13");
...
test("older package invocation keeps a newer compatible current pointer", async () => {
  ...
  try {
    ...
    const newer = releaseMetadata({ version: "0.4.13" });
    ...
    const calls = [];
    const fetchImpl = async (url, options) => {
      const metadata = url.includes("v0.4.13") ? newer : older;
...
test("core update plan binds signed source and apply switches only after verification", async () => {
  ...
  try {
    ...
    const fetchImpl = async (url) => {
      ...
      const useSecond = url.includes("/v0.4.13/");
    ...
    assert.equal(plan.target_version, "0.4.13");
    ...
    assert.equal(receipt.active_version, "0.4.13");
    ...
    assert.equal(JSON.parse(fs.readFileSync(paths.currentPath, "utf8")).version, "0.4.13");
...
test("update state is shared-schema, lazy, offline-safe, and signature fail-closed", async () => {
  ...
  try {
    ...
    assert.equal(ignored.state.latest_version, "0.4.13");
    assert.deepEqual(ignored.state.ignored_versions, ["0.4.13"]);
