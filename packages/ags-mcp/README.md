...
exactly `ags_decide` and `ags_apply`; `upgrade` and `update` use the same typed
plan/apply control plane as the `ags` CLI, not JavaScript subcommands.
...
Update checks are lazy and at most once every seven days. Rust owns the
migrated host-global `~/.ags/v3/upgrade-check.json`; an unsigned release index
is recorded as `unavailable` and is never treated as a verified update. The
MCP package does not run a separate notifier.
