...
- Executor (Claude Code): Stop-only EvoMap method capture. Evolver
  SessionStart recall, UserPromptSubmit recall, and PostToolUse signal
  detection are explicitly denied. The AGS project-memory SessionStart hook is
  separate and read-only.
- Planner (Codex/Cursor): pre-solution advisory recall only. It must not decide
  task level, execution authority, review gate, verification gate, or release
  boundary.
