...
fn reading_context_capsule_and_declaring_no_private_edits_passes() {
    let input = card_body(
            "路径：\n- /workspace/agent-governance-suite-private-rust\n\
             Executor: Claude Code\n\
             Runtime adapter: claude-code\n\
             Execution surface: cli\n\
             Permission mode: execute-and-verify\n\
             Parallelism: none\n\
             任务级别：Medium\n\
             读取：\n- /home/tester/.agents/memory/projects/agent-governance-suite-private/context-capsule.md\n\
             任务：升级 Rust 实验舱 task-card-validator 规则能力\n\
             目标：在 Rust 实验舱内增加字段值、组合、质量和风险检查\n\
             非目标：不修改 /workspace/agent-governance-suite-private，不修改 /workspace/agent-governance-suite-stable，不提交，不推送\n\
             关键路径：\n- /workspace/agent-governance-suite-private-rust/crates/task-card-validator/src/lib.rs\n\
             验证：\ncargo fmt --check\ncargo test\n\
...
fn read_context_capsule_no_modify_passes_v3() {
    ...
    let input = card_body(
            "路径：\n- /workspace/agent-governance-suite-private-rust\n\
             ...
             任务：升级 Rust 实验舱校验器规则\n\
             目标：增加字段组合和保护边界检查\n\
             非目标：不修改 /workspace/agent-governance-suite-private，不修改 /workspace/agent-governance-suite-stable\n\
             关键路径：\n- crates/\n\
             验证：\ncargo test --workspace\n\
             停止条件：\ntest 失败时停止\n\
             交付：\n返回测试结果和修改摘要\n",
