...
pub fn validate(task_card: &str, report: &str) -> DeliveryClosureResult {
    let task_card_hash = receipt::sha256_hex(task_card.as_bytes());
    let delivery_report_hash = receipt::sha256_hex(report.as_bytes());
    let receipt_id = format!("receipt-{}", &task_card_hash[..12]);
    let mut checks = Vec::new();

    let card = match task_card_validator::parse_validated(task_card) {
        Ok(card) => card,
        Err(errors) => {
    ...
    let contract = task_card_validator::closure_contract(&card);
...
交付：\n- 输出交付报告\n"
    ...
    fn report(card: &str) -> String {
        let hash = receipt::sha256_hex(card.as_bytes());
        format!(
            "# 任务交付报告\n\
Closure schema: 1.0\n\
Contract ID: tc-0123456789abcdef\n\
