...
pub use checks::orchestration::run_checks;
pub use checks::resolution::{skill_resolution_coverage_check, skill_resolution_drift_check};
...
pub fn third_party_capability_routing_finding(
    verify: &ags_capability_governance::skill_body::console::HostVerifyResult,
) -> Finding {
    let gaps: Vec<String> = verify
        ...
        .filter(|check| {
            check.expected
                && check.visibility
                    != ags_capability_governance::skill_body::console::HostVisibilityStatus::Visible
        })
    ...
    for item in &plan.items {
        ...
        match item.check_name.as_str() {
            ...
            "scripts-chmod" => {
                ...
                let _script_path = Path::new(path_str);
                // Use std::fs::set_permissions to set executable bit on unix
                #[cfg(unix)]
                {
                    use std::os::unix::fs::PermissionsExt;
                    if let Ok(meta) = std::fs::metadata(_script_path) {
                        let mut perms = meta.permissions();
                        let mode = perms.mode();
                        perms.set_mode(mode | 0o111);
                        match std::fs::set_permissions(_script_path, perms) {
                            Ok(_) => repaired.push(format!(
                                "{}: chmod +x {}",
                                item.check_name,
                                _script_path.display()
                            )),
                            Err(e) => failed.push(format!("{}: {}", item.check_name, e)),
                        }
                    } else {
