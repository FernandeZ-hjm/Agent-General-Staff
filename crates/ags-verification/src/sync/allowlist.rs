...
use super::types::{DriftKind, ProjectKind};
        ...
        files: vec![
            ...
            FileAllowlistEntry {
                ...
                removable_content_patterns: vec![
                    "machine-specific-root/".to_string(),
...
pub fn is_allowed(
    ...
    let wildcard_allowed = !whole_file_wildcard_forbidden(file);
    for entry in &allowlist.files {
        ...
        match kind {
            ...
            DriftKind::SectionMissing | DriftKind::ExtraSection => {
                let path_str = super::types::format_section_path(section_path);
                if (wildcard_allowed && entry.allowed_missing_sections.iter().any(|s| s == "*"))
...
fn whole_file_wildcard_forbidden(file: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn public_rewrites_are_not_whole_file_allowlist_exceptions() {
        let al = default_public_allowlist();
        assert!(!is_allowed(
            &al,
            "CLAUDE.md",
        ...
        assert!(!is_allowed(
            ...
            "RELEASE_NOTES.md",
            &[
                "Agent Governance Suite Public Edition Release Notes".to_string(),
    ...
    #[test]
    fn wildcard_cannot_exempt_rust_workflows_or_scripts() {
        let al = Allowlist {
            description: "attempted broad rewrite".to_string(),
            files: [
    ...
    fn removable_line_detection() {
        ...
        assert!(is_removable_line(
            &al,
            "protocol/runtime-adapters.md",
            "some text with machine-specific-root/ path"
        ));
        assert!(!is_removable_line(
            &al,
            "protocol/runtime-adapters.md",
