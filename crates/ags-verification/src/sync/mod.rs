...
mod tests {
    ...
    fn write_public_manifest(source: &Path, target: &Path) {
        let workspace = Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .unwrap();
        let mut authority = self::manifest::load_public_payload_authority(workspace).unwrap();
        authority.projection_state = "frozen".to_string();
        let expected = self::manifest::public_payload_files(workspace).unwrap();
        for relative in expected {
            if relative == self::manifest::PUBLIC_PAYLOAD_AUTHORITY_PATH {
                continue;
            }
            let source_path = source.join(&relative);
            let fixture = fs::read(&source_path).unwrap_or_else(|_| {
                fs::read(workspace.join(&relative))
                    .unwrap_or_else(|_| format!("fixture:{relative}\n").into_bytes())
            });
        ...
        for rewrite in &mut authority.public_rewrites {
            let bytes = fs::read(target.join(&rewrite.path)).unwrap();
            rewrite.target_sha256 = ags_platform::sha256(&bytes);
        }
        for overlay in &mut authority.public_overlay_files {
            let bytes = fs::read(target.join(&overlay.path)).unwrap();
            overlay.target_sha256 = ags_platform::sha256(&bytes);
        }
        let rendered = serde_yaml::to_string(&authority).unwrap();
        for root in [source, target] {
            write_file(
                root,
                self::manifest::PUBLIC_PAYLOAD_AUTHORITY_PATH,
                &rendered,
            );
        }
    }
