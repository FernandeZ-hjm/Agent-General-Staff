...
//! - **Current version is supplied by the product adapter** — never hard-coded.
...
///
/// `refs/heads/release/latest` is a movable channel pointer maintained only
/// after a formal GitHub Release succeeds. It must resolve to exactly one strict
/// `vX.Y.Z` tag commit. This keeps the reset `v0.3.0` line independent from
/// historical `v2.x` tags without adding an HTTP client dependency.
...
/// `git ls-remote` probe. Drains stdout on a thread so the child
...
pub fn evaluate(runtime_home: &Path, current_version: &str) -> NotifyOutput {
...
pub fn notifier_status_json(runtime_home: &Path, current_version: &str) -> serde_json::Value {
...
pub fn notifier_status_line(runtime_home: &Path, current_version: &str) -> String {
    let state = read_state(runtime_home);
    match state {
...
dddddddddddddddddddddddddddddddddddddddd\trefs/tags/v0.3.1\n";
    ...
    fn disabled_short_circuits_without_network_or_state() {
        ...
        let out = evaluate(&home, env!("CARGO_PKG_VERSION"));
        assert!(!out.notify);
        assert_eq!(out.reason, "disabled");
        assert_eq!(out.current_version, env!("CARGO_PKG_VERSION"));
    ...
    fn due_with_older_or_equal_is_up_to_date() {
        ...
        let home = tmp("equal");
        std::env::set_var("AGS_UPDATE_FAKE_DATE", "2026-06-19");
        std::env::set_var("AGS_UPDATE_FAKE_LATEST", env!("CARGO_PKG_VERSION"));
        let out = evaluate(&home, env!("CARGO_PKG_VERSION"));
        assert!(!out.notify);
        assert_eq!(out.reason, "up-to-date");
        assert_eq!(
            out.latest_version.as_deref(),
    ...
    fn status_json_is_read_only() {
        ...
        let v = notifier_status_json(&home, env!("CARGO_PKG_VERSION"));
