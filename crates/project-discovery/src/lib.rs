...
//
// A full closure requires the requested host to read on start, capture on its
// native close event, archive completed task evidence, and execute installed
// bridge scripts. Evidence from one host never satisfies another.

const MEMORY_START_MARKER: &str = "context-memory-start";
const MEMORY_CAPTURE_MARKER: &str = "claude-stop-memory-capture";
const COMMON_MEMORY_SCRIPTS: &[&str] = &[
    "context-memory.sh",
    "context-memory-start.py",
    "claude-stop-memory-capture.py",
];
const CLAUDE_MEMORY_SCRIPTS: &[&str] = &[
    "context-memory.sh",
    "context-memory-start.py",
    "claude-stop-memory-capture.py",
    "raw-tool-call-stop-guard.js",
];
const OMP_MEMORY_EXTENSION: &str = "ags-memory-lifecycle.js";
...
fn resolve_project_slug(target: &Path) -> String {
...
fn extract_profile_slug(target: &Path) -> Option<String> {
...
fn settings_event_commands(settings_path: &Path, event: &str) -> Vec<String> {
...
fn capture_scripts_present(home: &Path, required: &[&str]) -> bool {
...
fn host_hook_commands(paths: &[PathBuf], event: &str) -> Vec<String> {
    paths
        .iter()
...
fn omp_extension_wired(paths: &[PathBuf]) -> bool {
    paths.iter().any(|path| {
...
pub fn render_session_preflight_text(preflight: &SessionPreflight) -> String {
    ...
    let lifecycle = &preflight.memory_lifecycle;
    lines.push(format!(
        "Lifecycle:       {} host={} adapter={} (read={}, write={}, archive={}, scripts={})",
        lifecycle.status,
        lifecycle.host,
        lifecycle.adapter,
        lifecycle.read_wired,
        lifecycle.write_wired,
        lifecycle.archive_ready,
        lifecycle.scripts_present
    ));
    lines.push(format!("  {}", lifecycle.summary));
    lines.push(String::new());
...
mod tests {
    ...
    fn ml_write_scripts(home: &Path) {
        let dir = home.join(".agents/scripts");
        std::fs::create_dir_all(&dir).unwrap();
        for name in CLAUDE_MEMORY_SCRIPTS {
    ...
    fn memory_lifecycle_full_when_files_hooks_and_scripts_present() {
        ...
        let lifecycle = compute_memory_lifecycle_at(&target, &home);
        assert_eq!(lifecycle.status, "full");
        assert!(
            lifecycle.files_present
                && lifecycle.read_wired
                && lifecycle.write_wired
                && lifecycle.scripts_present
                && lifecycle.archive_ready
...
"#,
        ...
        let lifecycle = compute_memory_lifecycle_at_for_host(
            &target,
            &home,
