...
fn imported_skill_distribution_is_dry_run_then_transactional_apply() {
    ...
    let body_root = ctx.home.join(".ags/private-runtime/skill-bodies");
...
#[test]
fn omp_inherited_codex_registry_is_never_live_visibility() {
    let (ctx, base) = ctx_with("ompinherited", canned_list());
    let inv = build_inventory(&ctx, &["omp"]);
    let visibility = &find(&inv, "ags").host_visibility[0];
    assert_eq!(visibility.status, HostVisibilityStatus::Degraded);
    assert!(visibility
        .evidence
        .iter()
        .any(|evidence| evidence.contains("configuration-source evidence only")));
    let _ = std::fs::remove_dir_all(&base);
}
