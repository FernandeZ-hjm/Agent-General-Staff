...
use ags_platform::{atomic_write, canonical_workspace_root, sha256_file};
    ...
    fn capability_reference(&self, target: &Path, host: &str) -> serde_json::Value {
        ...
        match self.read_catalog(&binding) {
            Ok(snapshot) => serde_json::json!({
                "uri": "ags://capabilities/current-host",
                "status": "ready",
...
fn load_capability_bundle(
    runtime_home: &Path,
    workspace: &Path,
) -> Result<HashMap<String, skill_resolver::HostCapabilitySnapshot>, String> {
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                let _connection = std::thread::spawn(move || {
                    ...
                    if let Err(error) =
                        handle_connection(stream, registry, state, shutdown, handler)
                    {
                        let _ = writeln!(
                            std::io::stderr(),
...
mod tests {
    ...
    fn corrupt_capability_bundle_fails_before_daemon_readiness() {
        ...
        let paths = ServicePaths::new(&runtime, &canonical).unwrap();
