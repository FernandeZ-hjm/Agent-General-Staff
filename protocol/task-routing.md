...
This document defines how Cursor should classify and execute TempoFlow development tasks.
...
3. Identify whether data, historical outputs, vector stores, migrations, or irreversible operations are involved.
4. Identify whether Cursor may directly edit files, should first return a short design note, or must wait for confirmation.
5. Decide whether to implement directly or delegate to Claude Code CLI.
6. Select only the skill tags that directly apply.
7. Define the narrowest meaningful verification command before editing.
...
- Curate an old PR_News vector library without breaking the baseline.
