...
此阶段仍不涉及 Light / Medium / Heavy 分级。方案只是方案，不是写入授权或任务卡。

### 3. Execution Decision Phase（执行决策）

- 用户确认方案后，方案成为后续执行决策的稳定输入，但确认本身不授权 mutation。
- 用户明确授权当前宿主在同一会话修改（如“开改”“按这个改”“开始实现”）时，进入
  `direct-edit`，不生成任务卡。
- 用户明确要求任务卡、交给其他 Agent、后台/跨会话/可恢复执行时，进入
  `task-card-handoff`；已确认方案成为任务卡的输入来源。
- 两种授权都没有时保持只读；独立高风险停止条件仍可阻断任一路径。

**硬规则：用户的原始自然语言请求 ≠ 可执行任务卡。** 不得把初始聊天消息直接当作
Light / Medium / Heavy 分级依据。`direct-edit` 依赖已确认方案与当前明确修改授权；
task-card handoff 依赖已确认方案与明确任务卡/交接指令。
...
### 3.5. Task-Card Handoff Gate（任务卡交接门槛）

**这是 task-card handoff 的生成门禁，不是同会话 direct-edit 的写入门禁。**

- 用户确认方案（"方案 OK"）只结束 Solution Phase，不授权 mutation。
- 用户明确发出任务卡/交接指令（"生成任务卡"、"按这个方案出任务卡"、"交给 Claude Code 执行"等）后，才允许生成可执行任务卡。
...
- Codex / Cursor 收到明确同会话修改授权时，走 `direct-edit`，不得为了本地执行而伪造任务卡请求。

**执行分流：方案确认 → 明确 direct-edit 授权或 task-card handoff 指令 → 对应执行路径。**
...
- **入口意图识别**：`ags gate prompt-request <request>`（或 MCP `ags_solution_check`
  ...
  分类器（`prompt-request-classifier`）。命中"给我提示词""生成提示词""生成任务卡"
  "交给 Claude Code""给 CC 执行""写个 prompt""让 Claude 做"等带动作的中英文
  触发词时 `decision=require_task_card`，宿主必须进入任务卡生成闭环。检测只是
  信号，不替代 task-card handoff 门槛——任务卡输出仍要求显式任务卡指令。
...
| Task-card handoff gate | 形态级 | 任务卡/交接指令 | 非 canonical 任务卡输出 |
...
- `direct-edit` 把分级用于当前宿主的风险、review 与 verification 决策，不生成任务卡
- `task-card-handoff` 生成任务卡并填入 `任务级别：` 字段

### 5. Gate / Execution / Receipt Phase（门禁 / 执行 / 收据）

- `direct-edit`：当前宿主按独立 stop/review/verification 边界执行并交付证据
- `task-card-handoff`：任务卡通过 validator 与 execution-policy resolver 后由 Executor 执行
- 两条路径完成后都输出 delivery report 和 receipt
...
**关键分工：** Codex / Cursor 负责 preflight → solution → execution decision → routing，
可在明确同会话授权后直接执行，也可生成 handoff task card；Claude Code 只消费 handoff
路径产出的任务卡，不得从原始用户自然语言请求自行推断任务级别。
