...
Cursor cannot directly inherit every external agent runtime tool, but it can reuse project rules, local skill files, and task protocols. In TempoFlow, Cursor is the primary development agent and should choose skills proactively.
...
- Paths in this document that contain `$HOME` are template paths. Before opening any SKILL.md, expand `$HOME` to the actual home directory of the current machine (e.g., `/Users/a92550` on Mac mini, `/Users/hujiaming` on dev machine).
...
- graphify-project-map: $HOME/.agents/skills/graphify-project-map/SKILL.md
...
- AGENTS.md
- CLAUDE.md
- .cursor/rules/tempoflow-primary-agent.mdc
