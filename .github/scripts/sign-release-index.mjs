...
const assetPattern = new RegExp(
  `^ags-v${escapedVersion}-(?:aarch64-apple-darwin|x86_64-apple-darwin|aarch64-unknown-linux-gnu|x86_64-unknown-linux-gnu)\\.tar\\.gz$|^ags-v${escapedVersion}-x86_64-pc-windows-msvc\\.zip$`,
  "u"
);
...
if (!crypto.verify(null, bytes, crypto.createPublicKey(privateKey), signature)) {
  throw new Error("release index self-verification failed");
}
if (
  crypto.createPublicKey(privateKey).export({ type: "spki", format: "pem" }).toString().trim() !==
  publicKey.trim()
) {
  throw new Error("release signing private key does not match the pinned public key");
}
