...
name: graphify
description: |
  Use this when the user asks for Graphify, `/graphify`, project knowledge graph,
  项目图谱, 项目知识图谱, 代码图谱, 架构图谱, or a graph-based project map.
  Builds or updates a Graphify knowledge graph, reads Graphify reports, and
  produces a structured project map. Do not use for ordinary code reading or
  general architecture explanation unless the user clearly wants a graph/map.
---
...
当用户明确提出 Graphify、`/graphify`、项目图谱、项目知识图谱、代码图谱、架构图谱、基于图谱的项目画像时使用。

不要因为普通的“理解项目”“看架构”“梳理模块”自动调用本技能；这些场景默认先用常规代码阅读、`zoom-out` 或项目协议。只有用户表达“图谱/Graphify/项目地图/知识图谱”这类图谱化意图时，才进入本技能。
